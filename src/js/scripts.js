...
NioApp.Dropdown = {
  ...
  extended: function(elm,active){
    ...
    elm.addEventListener('mouseenter', (event) => {
      if(NioApp.Win.width > NioApp.Break[navbarCollapse]){
...
NioApp.Tags = function(selector){
  ...
  if( elm.length > 0 ){
    elm.forEach(item => {
      let cross = item.dataset.cross ? JSON.parse(item.dataset.cross) : true;
      let plainText = item.classList.contains('form-control-plaintext');
      let containerInner = `${plainText ? 'choices__inner-plaintext' : ''}`;
      const choices = new Choices(item, {
        silent: true,
        allowHTML: false,
        placeholder: true,
        ...
        removeItemButton: cross,
        classNames: {
          containerInner: ['choices__inner', ...containerInner ],
        }
      });
    })
  }
